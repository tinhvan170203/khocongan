    ...
    const toast = (data) => {
      ...
      let totalPage2 = 0;
        ...
        const searchLichsu = async (data) => {
            try {
              let res = await axios.get(`/quan-tri/${id}/lich-su-thu-hoi/search`, {
                params: data,
              });
              ...
              result.forEach((item, index) => {
                ...
                $("#bang2").append(`
                                    <tr id="${item._id}">
                                        ...
                                        <td style="font-weight:bold;text-align: center">${item.donvi.tendonvi}</td>
                                        <td style="text-align: center">${item.canbothuhoi}</td>
                                        <td style="text-align: center">${item.canbobangiao}</td>
                                        <td style="text-align: center">${item.khothuhoi.tenkho}</td>
                                        <td style="text-align: center;">${item.ghichu}</td>
                                        <td style="text-align: center">
                                            <button class="btn1 btn-edit"><a href="/quan-tri/${id}/thu-hoi/chi-tiet/${item._id}" target="blank"><i class="far fa-eye btn-icon" title="Xem chi tiết""></i></a></button>
                                            <button class="btn1 btn-edit"><a href="/quan-tri/${id}/chinh-sua-thu-hoi/${item._id}"><i class="far fa-edit btn-icon" title="Chỉnh sửa"></i></a></button>
      ...
      const deleteLichsuthuhoi = async (id1) => {
        await axios.delete(`/quan-tri/${id}/lich-su-thu-hoi/${id1}`);
        $('#addForm').submit();
        toast("Xóa lần xuất kho thành công !");
      };
    
      $("#confirm").on("click", async () => {
        await deleteLichsuthuhoi(id_Delete);
        $(".close").click();
      });
