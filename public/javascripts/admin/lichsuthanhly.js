    ...
    const toast = (data) => {
      ...
      let totalPage2 = 0;
        ...
        const searchLichsu = async (data) => {
            try {
              let res = await axios.get(`/quan-tri/${id}/lich-su-thanh-ly/search`, {
                params: data,
              });
              ...
              result.forEach((item, index) => {
                ...
                $("#bang2").append(`
                                    <tr id="${item._id}">
                                        ...
                                        <td style="text-align: center">${item.canbothanhly}</td>
                                        <td style="text-align: center;">${item.ghichu}</td>
                                        <td style="text-align: center">
                                            <button class="btn1 btn-edit"><a href="/quan-tri/${id}/thanh-ly/chi-tiet/${item._id}" target="blank"><i class="far fa-eye btn-icon" title="Xem chi tiết""></i></a></button>
                                            <button class="btn1 btn-edit"><a href="/quan-tri/${id}/chinh-sua-thanh-ly/${item._id}"><i class="far fa-edit btn-icon" title="Chỉnh sửa"></i></a></button>
      ...
      const deleteLichsuthanhly = async (id1) => {
        await axios.delete(`/quan-tri/${id}/lich-su-thanh-ly/${id1}`);
        $('#addForm').submit();
        toast("Xóa lần thanh lý thành công !");
      };
    
      $("#confirm").on("click", async () => {
        await deleteLichsuthanhly(id_Delete);
        $(".close").click();
      });
