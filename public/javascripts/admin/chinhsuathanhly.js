$(document).ready(async () => {
    ...
    const id_lichsu = $(".menu-head").attr("id");
          ...
          data.hanghoaNoneSeriInKho.forEach((item, index) => {
            $('#table-body').append(`
              <tr id="${item._id}" class="no-seri">
                ...
                <td><button style="text-align: center;width: 100%;padding: 2px" class="btn btn-success export" title="Thêm vào danh sách thanh lý"><i class="fas fa-shopping-cart"></i></button></td>
                </tr>
            `)
          })
        };
    ...
    table1 = $("#table1").DataTable({
      ...
      language: {
        search: "Tìm kiếm",
        sInfoEmpty: "",
        sEmptyTable: "Không có dữ liệu hàng hóa thanh lý",
        sInfoFiltered: "",
        sInfo: "Tổng cộng  _TOTAL_ loại hàng hóa thanh lý",
        sLengthMenu: "",
      },
      destroy: true, // cho phép hủy bỏ table tạo table mới với cùng id table
    });
    ...
    const thanhly = async (data) => {
      await axios.put(`/quan-tri/${id}/chinh-sua-thanh-ly/${id_lichsu}`,data)
    };
    ...
    $("#export").on("submit", async function (e) {
        ...
        toast('Cập nhật thanh lý hàng hóa thành công !')
    });
