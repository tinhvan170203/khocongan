$(document).ready( async() => {
  ...
  const addKho = async (data) => {
    try {
      let res = await axios.post(`/quan-tri/${id}/kho/add`, data);
      await tableKho();
      toast(`Thêm mới kho ${res.data} thành công !`);
      $("#addForm")[0].reset();
    } catch (error) {
  ...
  const tableKho = async () => {
    table =  await $("#table").DataTable({
      ajax: {
        url: `/quan-tri/${id}/kho/fetch`,
        dataSrc: "",
      },
      bSort: false,
      lengthMenu: [10],
      ...
      aoColumns: [
        ...
        {
          mData: function (data, type, dataToSet) {
            return `<p style="text-align: center" class="kyhieu" id=${data._id}>${data.tenkho}</p>`;
          },
        },
        {
          mData: function (data, type, dataToSet) {
            return `<p style="text-align: center" class="ten">${data.tenchitiet}</p>`;
          },
        },
        {
          mData: function (data, type, dataToSet) {
            return `<p style="text-align: center" class="vitri">${data.vitri}</p>`;
          },
        },
        {
          mData: function (data, type, dataToSet) {
  ...
  const deleteKho = async (id1) => {
    await axios.delete(`/quan-tri/${id}/kho/${id1}`);
    toast("Xóa kho thành công !");
    await tableKho();
  };

  $("#confirm").on("click", async () => {
    await deleteKho(id_Delete);
    $(".close").click();
  });
  ...
  const editKho = async (data) => {
    await axios.put(`/quan-tri/${id}/kho/${id_Edit}`, data);
    await tableKho();
