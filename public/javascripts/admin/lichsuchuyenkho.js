    ...
    const toast = (data) => {
      ...
      let totalPage2 = 0;
        ...
        const searchLichsu = async (data) => {
            try {
              let res = await axios.get(`/quan-tri/${id}/lich-su-chuyen-kho/search`, {
                params: data,
              });
              ...
              result.forEach((item, index) => {
                ...
                $("#bang2").append(`
                                    <tr id="${item._id}">
                                        ...
                                        <td style="font-weight:bold;text-align: center">${item.khochuyendi.tenkho}</td>
                                        <td style="font-weight:bold;text-align: center">${item.khochuyenden.tenkho}</td>
                                        <td style="text-align: center">${item.canbochuyenkho}</td>
                                        <td style="font-weight:bold;text-align: center;color: red">${thoigian}</td>
                                        <td style="text-align: center;">${item.ghichu}</td>
                                        <td style="text-align: center">
                                        <button class="btn1 btn-edit"><a href="/quan-tri/${id}/chuyen-kho/chi-tiet/${item._id}" target="blank"><i class="far fa-eye btn-icon" title="Xem chi tiết""></i></a></button>
                                        <button class="btn1 btn-edit"><a href="/quan-tri/${id}/chinh-sua-chuyen-kho/${item._id}"><i class="far fa-edit btn-icon" title="Chỉnh sửa"></i></a></button>
      ...
      const deleteLichsuChuyenkho = async (id1) => {
        await axios.delete(`/quan-tri/${id}/lich-su-chuyen-kho/${id1}`);
        $('#addForm').submit();
        toast("Xóa lần chuyển kho thành công !");
      };
    
      $("#confirm").on("click", async () => {
        await deleteLichsuChuyenkho(id_Delete);
        $(".close").click();
      });
